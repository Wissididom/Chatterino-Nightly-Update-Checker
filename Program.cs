...
namespace ChatterinoNightlyUpdateChecker
{
    public static partial class Program
    {
        ...
        private const string WebhookAvatarUrl =
            "https://user-images.githubusercontent.com/41973452/272541622-52457e89-5f16-4c83-93e7-91866c25b606.png";

        private const string NightlyLink = "https://github.com/Chatterino/chatterino2/releases/tag/nightly-build";

        private const string ContentFormatString =
            "New Nightly Version (Updated: <t:{0}:F>):\nLatest Commit: {1} by {2}\nLink: <{3}>\nPull Request: {4}";
        ...
        public static async Task Main(string[] args)
        {
            ...
            var commits = JsonSerializer.Deserialize<ListCommitsResponse>(await client
                .GetStringAsync("https://api.github.com/repos/Chatterino/chatterino2/commits?sha=nightly-build")
            ...
            if (File.Exists("lastUpdatedValue"))
            {
                ...
                else
                {
                    ...
                    {
                        commitString = commit.HtmlUrl is null
                            ? $"``{commitTitle}``"
                            : $"[``{commitTitle}``](<{commit.HtmlUrl}>)";
                    }

                    var coauthors = new List<string>();
                    if (!string.IsNullOrEmpty(description))
                    {
                        coauthors.AddRange(description.Split('\n').Where(line => line.StartsWith("Co-authored-by:"))
                            .Select(line => line["Co-authored-by:".Length..].Trim()).Select(coauthor =>
                            {
                                var i = coauthor.IndexOf('<', StringComparison.InvariantCulture);
                                return i >= 0 ? coauthor[..i].Trim() : coauthor;
                            }));
                    }
                    ...
                    {
                        author = commit.Author?.HtmlUrl is null
                            ? $"``{authorName}``"
                            : $"[{authorName}](<{commit.Author.HtmlUrl}>)";
                    }
                    ...
                    if (authorName != "dependabot" && authorName != "dependabot[bot]")
                        Console.WriteLine(
                            (await PostDiscordMessage(client, timestamp, commitString, author, description))
                            .StatusCode);
                }
        ...
        private static async Task<HttpResponseMessage> PostDiscordMessage(HttpClient client, string timestamp,
            string commit, string author, string description)
        {
